...
#        ad_times = [38, 53, 67, 99, 129, 288, 304, 334, 365, 384]
        ...
        #From start to first scene change = unclassified
...
#            #Pretty output
            ...
            div = (times[i] + times[i + 1]) / 2 #We recognize fingerprints from somewhere middle of the song
            
            if ((ad is True) and times[i] > times[i + 1]): #Skip the shot change detected in an ad sequence
            
                #Just making next time = prev time, because when ad was detected, prev_time = end of ad segment
                times[i + 1] = times[i] 
                i += 1
                continue
